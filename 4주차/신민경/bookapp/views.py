...
class BookReviewDetailView(APIView):
    def get(self, request, id, review_id):
        ...
        try:
            book = Book.objects.get(id=id)
            review = Review.objects.get(id=review_id, book=book)
        except Book.DoesNotExist:
            return Response({'message': '도서의 리뷰를 찾지 못했습니다.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request, id, review_id):
        ...
        try:
            book = Book.objects.get(id=id)
            review = Review.objects.get(id=review_id, book=book)
        except (Book.DoesNotExist, Review.DoesNotExist):
            return Response({'message': '도서의 리뷰를 찾지 못했습니다.'}, statue=status.HTTP_400_BAD_REQUEST)
    ...
    def delete(self, request, id, review_id):
        ...
        review.delete()
        return Response(status=status.HTTP_404_NOT_FOUND) # 이 상태 코드가 맞나..
